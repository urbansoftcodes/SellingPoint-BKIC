...
{
    public partial class TravelChangeMemberEndorsement : System.Web.UI.Page
    {
        ...
        public TravelChangeMemberEndorsement()
        {
        ...
        public void PopUpCon(object sender, EventArgs e)
        {
            EndorsementOperation(sender, "delete");
        }
        ...
        public void SetDependantDOB()
        {
            foreach (GridViewRow row in Gridview1.Rows)
            {
                ...
                if (relation.SelectedIndex > 0)
                {
                    ...
                    //if(relation.SelectedIndex == 1 || relation.SelectedIndex == 2)                    {
                    ...
                    //    ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "Set100Years(" + id + ");", true);
                    //}
                    if(relation.SelectedItem.Text == "self")
                    {
                        dob.Text = selectedDOB;
                        dob.Enabled = true;                        
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "SetAbove21Years(" + id + ");", true);  
                    }
                    else
                    {
                        dob.Text = selectedDOB;
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if(Convert.ToInt32(dt.Rows[i]["ItemSerial No"]) == 1)
                        {
        ...
        private void SetEndorsementType(BKIC.SellingPoint.DTO.RequestResponseWrappers.TravelEndorsement postTravelEndorsement, BKIC.SellingPoint.DTO.RequestResponseWrappers.TravelInsurancePolicy response)
        {
            ...
            postTravelEndorsement.PolicyCommencementDate = response.InsuranceStartDate.Value;
            postTravelEndorsement.ExpiryDate = response.ExpiryDate.Value;
            postTravelEndorsement.Remarks = "";
            postTravelEndorsement.AccountNumber = "";
            postTravelEndorsement.EndorsementType = "ChangeMemberDetails";
